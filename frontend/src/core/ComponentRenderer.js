...
// In memory structure for temp data
const components = {};
...
function getComponentClassByClassPath(classPath) {
  ...
  for (const cls of classPath)
    if (cls && registeredComponents[cls]) Component = registeredComponents[cls];
...
export function findRef(configuration, refId) {
  if (configuration.id === refId) return configuration;
  else if (configuration.children && configuration.children.length > 0)
    for (let childConfig of configuration.children) {
...
function getComponentByConfiguration(configuration, globalConfiguration, rootOptions, bus) {
  const { children: configChildren, refs, classPath, ...configurationRest } = configuration;
  const Component = getComponentClassByClassPath(classPath);

  // If no component's class is available
  if (!Component) return null;

  const children =
    configChildren && configChildren.length > 0
      ? configChildren.map((child) =>
          getComponentByConfiguration(child, globalConfiguration, null, bus)
        )
      : null;
...
export function ComponentRenderer({ parameters, configuration, rootOptions, refresh }) {
  ...
  const bus = {
    ...
    submitAndHash(hash) {
      ...
      components[configuration.id] = { nextHash: hash };
      this.submit();
    },
  };

  if (configuration && configuration.id && !components[configuration.id])
