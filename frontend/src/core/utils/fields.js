...
export function formatRef(fieldInfo, fieldValue) {
  ...
  if (subFields.length === 0) return 'Empty value';

  // If is available a field 'description' return it
  if (fieldValue.description) return `${fieldValue.description}`;

  const candidateDescriptiveFields = subFields.filter(
    (sF) =>
      typeof fieldValue[sF] === 'string' &&
      // Exclude dates
      isNaN(Date.parse(fieldValue[sF])) &&
      // Exclude Numbers
      isNaN(Number(fieldValue[sF])) &&
      sF !== 'password' &&
      (!fieldInfo.subRef || sF !== '_id')
  );

  if (candidateDescriptiveFields.length > 0) {
    // Is _id between descriptive fields ?
    const id = candidateDescriptiveFields.find((sF) => sF === '_id');

    // Is code between descriptive fields ?
    const code = candidateDescriptiveFields.find((sF) => sF === 'code');

    return (
      // If _id exists, take it as first and underline it
      (id ? `｟ ${fieldValue._id} ｠ ` : '') +
      // If code exists, take it as first and underline it
      (code ? `｟ ${fieldValue.code} ｠ ` : '') +
      // Take all strings subFields to create a description for the field
      candidateDescriptiveFields
        .filter((sF) => sF !== 'code' && sF !== '_id')
        .map((sF) => fieldValue[sF])
        .join(' / ')
    );
...
export function formatObject(fieldInfo, fieldValue) {
  return `{ ${Object.keys(fieldValue)
    .filter((k) => k !== '_id')
    .map((k) => `(${formatField(fieldInfo.type.obj[k], fieldValue[k])})`)
    .join(', ')} }`;
}
...
export function formatField(fieldInfo, fieldValue) {
  ...
  if (fieldInfo.ref || fieldInfo.subRef) return formatRef(fieldInfo, fieldValue);
  else if (fieldInfo.enum) return formatEnum(fieldInfo, fieldValue);
  else if (fieldInfo.kind === 'Date') return new Date(fieldValue).toLocaleString();
  else if (fieldInfo.kind === 'Array') return formatArray(fieldInfo, fieldValue);
  else if (fieldInfo.kind === 'Object') return formatObject(fieldInfo, fieldValue);
...
export function getFieldDefaultValue(fieldInfo) {
  const { primitive, reference, object, enumerator, array } = getFieldType(fieldInfo);

  if (fieldInfo._default !== undefined) {
    const _default = eval(fieldInfo._default);

    // if is a function then return the execution of it
  ...
  else if (enumerator) return null;
  else if (array) return [];
  else if (object) {
    // This is a single nested schema
    if (ɁɁ(fieldInfo, 'type', 'obj')) return getFieldDefaultValue(fieldInfo.type);
    else return {};
  } else if (primitive) {
    // Primitive
...
export function getFieldType(fieldInfo) {
  ...
  const arrayOfObjects =
    array && !fieldInfo.type[0].kind && !fieldInfo.type[0].ref && !fieldInfo.type[0].subRef;
