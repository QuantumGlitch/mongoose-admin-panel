 ...
 */
export default function ListDefault({ fieldInfo, fieldValue, onChange, validationError }) {
  ...
  // We know that fieldInfo is of type = 'Array' of consequence we will check
  // the type of the list's elements (by the property type[0])
  const { arrayOfObjects, subFieldsInfo } = getFieldType(fieldInfo);

  let elementsFieldsInfo = null;

  // List of objects
  if (arrayOfObjects) {
    elementsFieldsInfo = subFieldsInfo;
    if (fieldInfo.readOnly)
      elementsFieldsInfo = elementsFieldsInfo.map((f) => ({ ...f, readOnly: true }));
  }
  // List of primitives
  else
    elementsFieldsInfo = [
      { name: fieldInfo.name, ...subFieldsInfo, ...(fieldInfo.readOnly ? { readOnly: true } : {}) },
    ];
  ...
  return (
    <>
      <Detail
        ...
        currentElement={
          // Force refresh of detail if list[currentElementIndex] is an object
          arrayOfObjects
            ? list[currentElementIndex]
              ? { ...list[currentElementIndex] }
              : undefined
            : list[currentElementIndex]
        }
        validationError={
          ...
          validationError.children[currentElementIndex]
        }
