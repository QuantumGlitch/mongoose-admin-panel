...
export default class API {
  ...
  static headers() {
    return {
      ...
      ...(Auth.isLogged ? { Authorization: `Bearer ${Auth.accessToken}` } : {}),
    };
  }
  ...
  static create(
    ...
    return new Promise((resolve, reject) => {
      let req =
        method === 'post'
          ? API.post(url, parameters, cacheEnabled, true, true)
          : API.get(url, parameters, cacheEnabled, true, true);

      // errorHandled = true: manage errors with Messages' dialogs
      if (errorHandled)
        req
          .then((response) => {
            if (response.data.ok) {
          ...
          .catch((error) => {
            Messages.error({ error: error.message || error });
            if (reject) reject(error);
          });
      //normal request
      else
        req
          .then((response) => {
            if (response.data.ok) resolve(rawResponse ? response : response.data);
  ...
  static error(error) {
    ...
    else for (var x in error) if (typeof error[x] === 'string') return error[x];
