...
export function useLoadable({
  ...
  const load = useCallback((url, method, parameters, cacheEnabled, errorHandled) => {
    if (state.loading) return;
    dispatch({
      type: 'loading'
    });
    return API.create(url, method, parameters, cacheEnabled, errorHandled, rawResponse).then(data => dispatch({
      type: 'loaded',
      data
  ...
  }, [url, method, parameters, cacheEnabled, errorHandled, rawResponse]); // reload the request

  const refresh = useCallback(params => {
    if (!params) params = {}; // defaulting parameters if no value is present
    ...
    return load(params.url, params.method, params.parameters, params.cacheEnabled, params.errorHandled, params.rawResponse);
  }, [load]); // initialization of the useLoadable

  useEffect(() => {
    if (state.reload && url) load(url, method, parameters, cacheEnabled, errorHandled, rawResponse);
  }, [state.reload]); // if url parameters change (and is active) must trigger new load request after the current has been loaded
