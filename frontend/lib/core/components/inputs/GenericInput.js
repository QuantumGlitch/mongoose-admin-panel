...
export default function GenericInput(props) {
  ...
  const {
    ...
    array,
    object
  } = getFieldType(props.fieldInfo);
  /**
   * Choose by complex criterias
   */

  let ComplexInput = null; // This is a particular type of reference
  // Single file upload or multiple

  if (kind === 'File' || array && props.fieldInfo.type[0].kind === 'File') ComplexInput = registeredComplexInputs.File;else if (reference) {
    // This is used when referencing a subDocument of a reference
    if (boundTo) ComplexInput = registeredComplexInputs.BoundRef;else ComplexInput = registeredComplexInputs.Ref;
  } else if (object) ComplexInput = registeredComplexInputs.Object;else if (array) ComplexInput = registeredComplexInputs.List;else if (enumerator) ComplexInput = registeredComplexInputs.Enum;
  if (ComplexInput) return /*#__PURE__*/React.createElement(ComplexInput, props);
  /**
   * Choose by kind
   */
  ...
  if (registeredInputs[kind]) {
    ...
    return /*#__PURE__*/React.createElement(InputByKind, props);
  } else return /*#__PURE__*/React.createElement(DefaultInput, props);
}